...
int test_should_load_file() {
  ...
  save_file(&db, "/tmp/loaddb.sep");

  Database loaded;

  int load_result = load_file(&loaded, "/tmp/loaddb.sep");

  if (load_result != 0) {
    printf("\n\tLoad failed. Cannot continue.");
    res |= 1;
  } else {
    if (loaded.table_count != 2) {
      printf("\n\tDB does not have 2 tables");
      res |= 1;
    }

    if (loaded.tables[0].row_count != 1) {
      printf("\n\tTable A missing row");
      res |= 1;
    }

    if (loaded.tables[0].rows[0].field_count != 1) {
      printf("\n\tRow is missing data from Table A");
      res |= 1;
    }

    if (loaded.tables[1].row_count != 1) {
      printf("\n\tTable B missing row");
      res |= 1;
    }

    if (loaded.tables[1].rows[0].field_count != 1) {
      printf("\n\tRow is missing data from Table B");
      res |= 1;
    }
  }
